...
void LayerPlan::addExtrusionMove(Point p, const GCodePathConfig& config, SpaceFillType space_fill_type, float flow, bool spiralize, double speed_factor, float fan_speed)
{
        ...
        if (flow >= wall_min_flow && (first_line || !travel_required || vSize2f(p0 - p1) > max_spurious_fat_segment_length2))
        {
            ...
            if (is_small_feature)
            {
                ...
                addExtrusionMove(p1, non_bridge_config, SpaceFillType::Polygons, flow, spiralize,
                    (!overhang_in_layer || (overhang_in_layer && small_feature_speed_factor < overhang_speed_factor)) ? small_feature_speed_factor : overhang_speed_factor
                );
            }
            else
            {
                addWallLine(p0, p1, mesh, non_bridge_config, bridge_config, flow, non_bridge_line_volume, (overhang_in_layer) ? overhang_speed_factor : speed_factor, distance_to_bridge_start);
            }
        }
        else
        {
    ...
    for(unsigned int extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        for(unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            if (!spiralize) // normal (extrusion) move (with coasting
            {
                ...
                const float path_fan_speed = path.getFanSpeed();
